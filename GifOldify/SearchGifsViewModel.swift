...
class SearchGifsViewModel {
    ...
    func searchGifs(start: Bool, query: String) -> Observable<[Gif]> {
        return Observable<[Gif]>.create({ (observer) -> Disposable in
            ...
            if start { self.resetPagination() }
            NetworkManager.shared().get("gifs/search?api_key=7cc07a1ed64e48bab974610f0c97e8a1&q=\(encodedQuery)&limit=\(self.pagingParams.limit)&offset=\(self.pagingParams.offset)", parameters: nil, progress: nil, success: { (task, response) in
